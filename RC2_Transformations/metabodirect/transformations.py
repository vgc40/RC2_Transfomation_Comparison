...
def get_keys(keyfile):
    ...
    # Support new key file format: Name, Mass, Biotic/abiotic
    if 'Name' in key.columns and 'Mass' in key.columns:
        key['Formula'] = key['Name']
        key = key.rename(columns={
            'Name': 'Transformation',
            'Mass': 'mf',
            'Biotic/abiotic': 'Group',
        })
        if 'Group' not in key.columns:
            key['Group'] = 'Unknown'

    key['mf'] = pd.to_numeric(key['mf'], errors='coerce')
    key = key.dropna(subset=['mf'])
...
# --------------------------------------------------
def calculate_transformations(df, keys, path, err_thresh=0.000010):
    import bisect
    import operator
    ...
    for sample in df_transf.columns[1:]:  # Skip mass column
        ...
        if len(result_tuples) == 0:
            ...
            result_df.to_csv(filename, index=False, float_format='%.9f')
...
def summarize_transformations(path):
    ...
    summary_transf.to_csv(filename, index=False, float_format='%.9f')
    return
...
def get_node_table(df, path):
    ...
    node_table.to_csv(filename, index=False, float_format='%.9f')
...
def create_cytoscape_network(node_table, path):
    ...
    node_table['Mass'] = round(node_table['Mass'], 9)
    ...
    for ii, file in enumerate(edge_files):
        ...
        edge_table['Feature_X'] = round(edge_table['Feature_X'], 9)
        edge_table['Feature_X'] = edge_table['Feature_X'].astype(str)
        edge_table['Feature_Y'] = round(edge_table['Feature_Y'], 9)
